...
def attn_single(attn_states, fea_size, seq_len=300):
  with tf.variable_scope("attention"):
    k = tf.get_variable(
        "attn_w", [1, 1, fea_size, 100])
    v = tf.get_variable("attn_v", [100])
    hidden = tf.reshape(attn_states,
                                [-1, seq_len, 1, fea_size])
    hidden_features = tf.nn.conv2d(hidden, k, [1, 1, 1, 1], "SAME")
    s = tf.reduce_sum(v * tf.nn.tanh(hidden_features), [2, 3])
    a = tf.nn.softmax(s)
    d = tf.reduce_sum(
        tf.reshape(a, [-1, seq_len, 1, 1]) * hidden, [1, 2])
    new_attns = tf.reshape(d, [-1, fea_size])
    return new_attns
...
class GRUAttnModelNew(models.BaseModel):
  def __init__(self):
    super(GRUAttnModelNew, self).__init__()
    self.cell_size = 1024
    self.max_steps = 300
  ...
  def two_attn_layers(self, model_input, vocab_size, num_frames,
    ...
    with tf.variable_scope("EncLayer0"):
      ...
      for i in xrange(num_splits):
        ...
        with tf.variable_scope("Attn"):
          enc_outputs = attn(enc_outputs, fea_size=1024, num_heads=1, seq_len=num_splits)
        if is_training:
    ...
    with tf.variable_scope("Attn"):
      flatten_outputs = attn(enc_outputs, fea_size=1024, num_heads=1, seq_len=num_splits)
    # TODO
    if is_training:
  ...
  def two_layers(self, model_input, vocab_size, num_frames,
    ...
    with tf.variable_scope("EncLayer1"):
      ...
      enc_outputs, enc_state = tf.nn.dynamic_rnn(
          second_enc_cell, first_layer_outputs, initial_state=enc_init_state, scope="enc1")
    flatten_outputs = attn_single(enc_outputs, fea_size=1024, seq_len=num_splits)
    # TODO
    if is_training:
      flatten_outputs = tf.nn.dropout(flatten_outputs, 0.7)
    logits = moe_layer(flatten_outputs, vocab_size, 2, act_func=tf.nn.sigmoid, l2_penalty=1e-8)
    return {"predictions": logits}
  ...
  def three_layers(self, model_input, vocab_size, num_frames,
    ...
    with tf.variable_scope("EncLayer0"):
      ...
      flatten_outputs = attn(enc_outputs, fea_size=1024, seq_len=30)
