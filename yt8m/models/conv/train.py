...
class Inception():
  def __init__(self, videos, labels):
    ...
    self.tf_global_step = slim.get_or_create_global_step()
    ...
    if self.phase_train:
      ...
      if 'AuxLogits' in end_points:
        self.aux_loss = 0.4 * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=end_points['AuxLogits'], labels=self.labels,))
        # self.aux_loss = tf.constant(0., name="aux_loss")
        total_loss += self.aux_loss
      self.main_cls_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
  ...
  def get_init_fn(self, sess, checkpoint_path):
