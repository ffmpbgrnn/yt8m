...
def AddGlobalStepSummary(summary_writer,
  ...
  if summary_writer and examples_per_second != -1:
    summary_writer.add_summary(
        MakeSummary("GlobalStep/" + summary_scope + "_Example_Second",
                    examples_per_second), global_step_val)
  if summary_writer:
    summary_writer.flush()
  info = ("global_step {0} | Batch Hit@1: {1:.4f} | Batch PERR: {2:.4f} | Batch Loss: {3:.4f} "
          "| Examples_per_sec: {4:.4f}").format(
...
def AddEpochSummary(summary_writer,
  ...
  info = ("epoch/eval number {0} | Avg_Hit@1: {1:.4f} | Avg_PERR: {2:.4f} "
          "| MAP: {3:.4f} | GAP: {4:.4f} | Avg_Loss: {5:3f}").format(
...
def GetListOfFeatureNamesAndSizes(feature_names, feature_sizes):
  ...
  list_of_feature_names = [
      feature_name.strip() for feature_name in feature_names.split(',')]
  list_of_feature_sizes = [
      int(feature_size) for feature_size in feature_sizes.split(',')]
